...
Provides async telnet connectivity with session management and persistent disk logging.
"""
...
@tool(
    name="telnet_client",
    description="Connect to a Telnet server, run commands, and return output. Best for line-based command shells. For interactive full-screen apps (games, editors), use raw_input=True and send single-key commands (e.g., 'q' to quit, not 'quit'). All session interactions are logged with timestamps.",
)
async def telnet_client_tool(
    ...
    Supports session persistence when running via HTTP/SSE transport, and logs
    all commands and responses to disk with ISO-8601 timestamps.

    Args:
        host: Host or IP to connect to
        port: Port number
        commands: List of commands to send
        telnet_session_id: Telnet session ID for reusing existing connections
        ...
        log_dir: Optional directory to store session log files (defaults to TELNET_LOG_DIR or ~/.mcp-telnet-logs)
        enable_logging: Whether to log session to disk (default: True)

    Returns:
        TelnetClientOutput with server responses, session info, and log file path
    """
    # Validate input
    try:
    ...
    logger.info(
        f"Session check: telnet_session_id={telnet_session_id}, active={session_active}, log={log_file_path}"
    )
...
async def telnet_close_session(session_id: str) -> SessionCloseResponse:
    ...
    if session:
        session_logger = TelnetSessionLogger()
        session_logger.log_session_end(
